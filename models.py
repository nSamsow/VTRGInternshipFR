...
from tensorflow.keras.layers import Conv2D, BatchNormalization, Activation, AveragePooling2D, MaxPooling2D, \
                                    GlobalAveragePooling2D, GlobalMaxPooling2D, Flatten, Dense, Input, Reshape, multiply
from tensorflow.keras.preprocessing.image import img_to_array
from tensorflow.keras.utils import get_file, get_source_inputs
from tensorflow.keras.models import Model

import utils
from utils import preprocess_input, _obtain_input_shape

# get labels
data_dir = os.path.join('C:/Users/60301712/.keras/datasets/lfw')
labels = [str(f) for f in os.listdir(data_dir)]
...
def detect_faces(image, detector, scale=1/4):
    frame = cv2.resize(image, None, fx=scale, fy=scale)  # original (480, 640)

    if detector == 'haar':
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        face_boxes = face_cascade.detectMultiScale(gray, 1.3)
    elif detector == 'mtcnn':
    ...
    for i in range(len(face_boxes)):
        face_boxes[i] = tuple([int((1/scale) * x) for x in face_boxes[i]])
    return face_boxes
...
class Models:
    def __init__(self, model):
        models = {
            'vgg16': VGG16(include_top=False, weights='vggface', input_shape=(224, 224, 3), pooling='max'),
            'resnet50': RESNET50(include_top=False, weights='vggface', input_shape=(224, 224, 3), pooling='max'),
            'senet50': SENET50(include_top=False, weights='vggface', input_shape=(224, 224, 3), pooling='max')}
        preprocess_version = {'vgg16': 1, 'resnet50': 2, 'senet50': 2}
        self.model = models[model]
        self.preprocess_version = preprocess_version[model]
...
def VGG16(include_top=False, weights='vggface', input_tensor=None,
          input_shape=(224, 224, 3),pooling='max', classes=None):
...
def RESNET50(include_top=False, weights='vggface', input_tensor=None,
             input_shape=(224, 224, 3), pooling='max', classes=None):
    input_shape = _obtain_input_shape(input_shape,
                                      default_size=224,
                                      min_size=32,
...
def SENET50(include_top=False, weights='vggface', input_tensor=None,
            input_shape=(224, 224, 3), pooling='max', classes=None):
    input_shape = _obtain_input_shape(input_shape,
                                      default_size=224,
                                      min_size=197,
